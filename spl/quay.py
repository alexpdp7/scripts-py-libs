...
def build_images_and_push(args):
    ...
    for tag in tags:
        ...
        utils.run_command(["podman", "build", ".", "--build-arg", f"QUAY_EXPIRES_AFTER={quay_expires_after}", "-t", f"{git.project_name()}:{tag}"])
        assert tag != "dirty", "attempting to push dirty image"
        utils.run_command(["podman", "push", f"{git.project_name()}:{tag}", f"quay.io/{QUAY_ORG}/{git.project_name()}:{tag}"])
    return tags
